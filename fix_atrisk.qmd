...
data_025 <-
  read_xlsx("r025.xlsx", col_types = c(rep("text", 28), rep("numeric", 8))) %>%
  rename(available = `Available Budget`) %>% 
  mutate(spend = Commitment + Obligation + Expenditure) %>% 
...
available <-
  ...
  #
  mutate(
    spend_addl_expected = (spend_todate / 10.0) * 2.0,
...
journal %>%
  distinct(from_costcenter_id, from_program_id, step) %>% 
  count(from_costcenter_id, from_program_id, sort = T) %>%
  filter(n > 1) %>% 
  arrange(from_costcenter_id, from_program_id)
```
